...
#        user = auth.authenticate(institute=institute, password=password , registrationapproved=True)
#        if user is not None:
...
def InstrumentView(request):
    ...
    for row in allInst:
        ...
        rowdumped = {"id" : row.id,"instrumentname":row.instrumentname,"category": row.category , "instrumentquantity" : row.instrumentquantity , "instrumentdescription" :row.instrumentdescription,"institute":row.institute,"image": "http://www.sweetwater.com/images/items/120/LPST5HTHDCH-medium.jpg?9782bd"}
